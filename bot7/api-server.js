...
const app = express()
const PORT = process.env.API_PORT || 3015; // Puerto diferente para la API
const BOT_ID = process.env.BOT_IDENTIFIER || 'bot7'
const QR_PATH = path.join(process.cwd(), 'bot_sessions', `${BOT_ID}.qr.png`);
const DEFAULT_QR_PATH = path.join(process.cwd(), 'bot.qr.png');

// Watcher para copiar el QR a una ubicación única
...
const startQRWatcher = () => {
    ...
    qrWatcherInterval = setInterval(() => {
        if (fs.existsSync(DEFAULT_QR_PATH)) {
            try {
                // Asegurar que la carpeta bot_sessions existe
                const sessionsDir = path.join(process.cwd(), 'bot_sessions');
                if (!fs.existsSync(sessionsDir)) {
                    fs.mkdirSync(sessionsDir, { recursive: true });
                }
                
                // Copiar el QR a la ubicación única
...
app.get('/qr', (req, res) => {
    // Intentar primero con el QR único, luego con el por defecto
...
app.get('/', (req, res) => {
    res.send(`
        ...
        <html>
        <head>
            <title>WhatsApp Bot 7 - QR Code</title>
            <meta charset="UTF-8">
